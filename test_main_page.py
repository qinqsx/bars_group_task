...
class TestAddingProductsToCart:
    def test_log_in_user_with_open_products_page(self, browser):
        ...
        login_page.open()
        login_page.log_in_user(LoginPageLocators.USER_NAME, LoginPageLocators.USER_PASS)
...
class TestSortingProducts:
    ...
    def test_sorting_products_price_low_to_high(self, browser):
        product_page = ProductsPage(browser, browser.current_url)
        before_sort_price_list = product_page.product_price_list_type_float()               #создаю переменную в которой хранится список цен до сортировки
        product_page.select_product_sort_by_index(2)
        after_sort_price_list = product_page.product_price_list_type_float()                #создаю переменную в которой хранится список цен после сортировки
        product_page.comparison_low_to_high(before_sort_price_list, after_sort_price_list)              #сравниваю отсортированную первую переменную со второй перменной отсортированной на сайте
